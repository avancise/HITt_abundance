...
wd <- "C:/Users/Amy/Google Drive/00 CRC/01 Tursiops abundance"
photoIDfile <- "TursiopsPhotoIDJune2020v2.csv"
...
#get coastline data
setwd("C:/Users/Amy/Google Drive/00 CRC/01 Tursiops abundance/Raw data/HI coastlines shapefile")
...
setwd("C:/Users/Amy/Google Drive/00 CRC/01 Tursiops abundance/Raw data/Hawaii_DepthContours")
...
#mapping visuals to manual generate subarea designations
# subdat <- capdat %>% filter(Area == "Maui Nui") %>%  mutate(Long=as.numeric(Long), Lat = as.numeric(Lat)) %>%
#   filter(!is.na(Long)) %>%
#   mutate(Long = {ifelse(.$Long > 0, (.$Long*-1), .$Long)}) %>%
...
#map only animals seen once
# map.1x.dat <- capdat %>% group_by(ID..) %>% filter(n() == 1) %>% ungroup() %>%
#   mutate(Long=as.numeric(Long), Lat = as.numeric(Lat)) %>%
#   filter(!is.na(Long)) %>%
#   mutate(Long = {ifelse(.$Long > 0, (.$Long*-1), .$Long)})
...
#   geom_sf(fill = "#00695C", alpha=0.5) +
#   coord_sf(crs = st_crs(4135)) +
#   geom_point(data=map.1x.dat, aes(x=Long, y=Lat, color=subarea)) +
#   theme_classic(base_size = 18) +
#   theme(legend.position = "none")